...
async def fetch_conflicting_events_for_meeting(state: AgentState):
    ...
    if bool_conflicting_events == False:
        return {"conflicting_events" : "NONE", "messages" : ["No conflicting events found, scheduling them..."], "events_to_be_scheduled" : state["events_to_be_scheduled"]}
    else:
...
async def resolve_conflicting_events(state: AgentState):
        ...
        ### Guidelines to interpret user input:
        - If the user in the input doesn't provide resolution for any specific event either by saying something like "Schedule all Events" or specifically referring the event by saying something like "For new meeting event..." then remove it from the output since the user don't want scheudle it
