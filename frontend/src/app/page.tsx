          ...
          <div className="bg-white/90 backdrop-blur-lg rounded-3xl shadow-xl p-8 space-y-6 border border-white/20 w-[98%] max-w-[1100px] mx-auto">
            {!scheduledMeetings ? (
              ...
              <div className="space-y-8">
                <div className="text-center animate-fadeIn">
                  <h2 className="text-lg bg-clip-text text-transparent bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600">
                    Scheduled following meetings:
                  </h2>
                </div>
                <div className="space-y-6">
                  {scheduledMeetings.map((meeting, index) => (
                    <div key={index} className="bg-white/90 backdrop-blur-sm rounded-xl px-6 py-4 shadow-xl border border-white/20">
                      <h4 className="font-medium text-slate-800 text-xl">{meeting.summary}</h4>
                      <div className="mt-4 space-y-2 text-sm text-slate-600">
                        <p className="flex items-center gap-2">📅 {formatDateTime(meeting.start)} - {formatDateTime(meeting.end)}</p>
                        <p className="flex items-center gap-2">📍 {meeting.location || 'No location specified'}</p>
                        <p className="flex items-center gap-2">👥 {meeting.attendees.join(', ')}</p>
                        <div className="mt-4 space-x-4">
                          <a href={meeting.event_link} target="_blank" rel="noopener noreferrer" 
                            className="text-blue-600 hover:text-blue-800">View in Calendar</a>
                          {meeting.meeting_link && (
                            <a href={meeting.meeting_link} target="_blank" rel="noopener noreferrer" 
                              className="text-blue-600 hover:text-blue-800">Join Meeting</a>
                          )}
                        </div>
                      </div>
                    </div>
